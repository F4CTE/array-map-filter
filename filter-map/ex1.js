...
function getJedisNames(characters) {
  return characters.filter(function(charact){
   return charact.side == 'light';})
  .map(function(charact){
      return charact.name
    })
}


console.log(getJedisNames([{ name: 'Yoda', side: 'light' },{ name: 'Count Dooku', side: 'dark' },{ name: 'Obiwan Kenobi', side: 'light' },{ name: 'Mace Windu', side: 'light' },{ name: 'Darth Sidious', side: 'dark' },{ name: 'Darth Maul', side: 'dark' }]))
